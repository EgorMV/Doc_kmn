...
Функция ЗаписатьЗакрытьНаСервере(ПометкаУдаления = Ложь)
	...
	ПоказанияПриборов.Вставить("ОстатокТопливаВыезд1"	, ОстатокТопливаВыезд1); // СД
	ПоказанияПриборов.Вставить("ОстатокТопливаВозврат1"	, ОстатокТопливаВозврат1); // СД
...
Процедура ПолучитьДанныеДокумента(Сервис, ЭтоНовыйДокумент)
	...
	ОстатокТопливаВыезд1 	= ДанныеДокумента.CountFuelStart1; // СД
	ОстатокТопливаВозврат1 	= ДанныеДокумента.CountFuelEnd1; // СД
...
Процедура ЗаполнитьСписокАвто(Сервис)
	...
	Для Каждого Сч Из АвтомобилиУАТ.Auto Цикл
		...
		НСтрока.РасходГСМпоНорме1 = Сч.ExpenseFuelNorm1; // СД
...
Функция ВыполнитьПроверкуЗаполнения()
	...
	Если Число(ОбщийПробег) <> (СпидометрВозврат - СпидометрВыезд) Тогда  // СД
		Ошибки.Добавить("Пробег по показаниям одометра и пробег по заданиям не совпадают!");
	КонецЕсли;
...
Процедура ЗаполнениеДопДанных(Событие = "")
	...
	Если ИскАвто.Количество()>0 Тогда
		РасходГСМпоНорме = ИскАвто[0].РасходГСМпоНорме;
		 ...
		 // СД
		РасходГСМпоНорме1 = ИскАвто[0].РасходГСМпоНорме1;
		ИтогОсн = 0;
		ИтогДоп = 0;
		ОбщийПробегНорма = 0;
		Для Каждого ТекСтрока Из ГСМ Цикл
			Если ТекСтрока.ТипТоплива = "СПБТ" Тогда
				ИтогОсн = ИтогОсн + ТекСтрока.Количество;
			Иначе
				ИтогДоп = ИтогДоп + ТекСтрока.Количество;
				// Прервать;
			КонецЕсли;
		КонецЦикла;
		РасходГСМпоНормеКМ = (ОстатокТопливаВыезд + ИтогОсн - ОстатокТопливаВозврат) / РасходГСМпоНорме * 100;
		РасходГСМпоНормеКМ1 = (ОстатокТопливаВыезд1 + ИтогДоп - ОстатокТопливаВозврат1) / РасходГСМпоНорме1 * 100;
		ОбщийПробегНорма = РасходГСМпоНормеКМ + РасходГСМпоНормеКМ1;
		// СД
