...
Процедура УстановитьОформлениеФормы(ВидДокументаНаименование)
	...
	Если ВидДокументаНаименование = "Командировка" Тогда
		...
		ПредставлениеHTML = СтрЗаменить(ПредставлениеHTML, "[]<br>", "");
		...
		МассДанных = ПолучитьДанныеСтроки("\[{.[^\]]*}\]", ПредставлениеHTML);
	...
	Иначе
		...
		// СД
		Элементы.ПроцессыСогласования.Видимость = Ложь;
...
Процедура КМН_КМН_ТаблицаПутевыеЛистыОбработкаВыбораВместо(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ЗаписатьПутевойЛист(ВыбранноеЗначение);	
...
&НаКлиенте
Процедура КМН_КМН_ТаблицаПутевыеЛистыПередУдалениемВместо(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура КМН_КМН_ТаблицаПутевыеЛистыОбработкаЗаписиНовогоВместо(НовыйОбъект, Источник, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ЗаписатьПутевойЛист(НовыйОбъект);
...
Процедура КМН_КМН_ИзменитьПутевойЛистВместо(Команда)
	...
	Если ТекСтрока = Неопределено ИЛИ ТекСтрока.ИД = "" Тогда
		Возврат;
	КонецЕсли;	
	ЗаполнитьПутевойЛист();
...
Процедура ЗаполнитьПутевойЛист(ЭтоНовыйДокумент = Ложь)
	...
	Если ЭтоНовыйДокумент Тогда
		...
		ТекСтрока = КМН_ТаблицаПутевыеЛисты.Добавить();
		ТекСтрока.ИД = ИДСтроки;
		Записать();
	Иначе	
	...
	ОткрытьФорму("Обработка.ПутевойЛистУАТ.Форма.Форма", ПараметрыФормы, Элементы.КМН_ТаблицаПутевыеЛисты, ИДСтроки);
...
&НаКлиенте
Процедура КМН_КМН_ОбновитьТаблицуАвансовыйОтчетРасходыВместо(Команда)
	
	ОбновитьТаблицуАвансовыйОтчетНаСервере();
	
КонецПроцедуры

&НаСервере
Функция ОбновитьТаблицуАвансовыйОтчетНаСервере()
	
	КМН_ТаблицаАвансовыйОтчетРасходы.Очистить();
	ЗагрузитьДопДанныеИзJSON("ДанныеАвансовыйОтчетРасходы", КМН_ТаблицаАвансовыйОтчетРасходы, Ложь);
...
Процедура ИзменитьСрокДействияДокументаПродолжениеВместо(Результат, Параметры) Экспорт 
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда 
		...
		СохранитьДопДанныеВЗначение(КМН_СрокКомандировки, "Срок командировки");
		ПроверкаСоответствияСуточных();
		ВывестиСуточныеДляОтчета();
...
//Параметры:
//ИмяДопРеквизита - наименование свойства
//РеквизитФормы - имя реквизита формы куда будет происходить загрузка
//Автозагрузка - признак использование метода при открытие формы
&НаСервере
Процедура ЗагрузитьДопДанныеИзJSON(ИмяДопРеквизита, РеквизитФормы, Автозагрузка = Истина)
	...
	Если ЗначениеЗаполнено(ДопРеквизит) Тогда
		...
		Если ИскСтрока <> Неопределено Тогда 
			Если ТипЗнч(ИскСтрока.Значение) = Тип("Строка") Тогда
				//уберем тяжелые строки из автозагрузки
				//Если СтрДлина(ИскСтрока.Значение)>5000 И Автозагрузка Тогда
				//	Возврат;
				//КонецЕсли;	
...
&НаКлиенте
Процедура ЗаписатьПутевойЛист(ДанныеПутевогоЛиста)
	
	Если ТипЗнч(ДанныеПутевогоЛиста) = Тип("Структура") Тогда
	
		ИскСтроки = КМН_ТаблицаПутевыеЛисты.НайтиСтроки(Новый Структура("ИД", ДанныеПутевогоЛиста.ИД));
		Если ИскСтроки.Количество() > 0 Тогда
			ТекСтрока = ИскСтроки[0];	
		Иначе 
			ТекСтрока = КМН_ТаблицаПутевыеЛисты.Добавить();
		КонецЕсли;
		
		Если ДанныеПутевогоЛиста.Свойство("ПометкаУдаления") Тогда
			КМН_ТаблицаПутевыеЛисты.Удалить(ТекСтрока);
		Иначе
			ТекСтрока.Автомобиль 	= ДанныеПутевогоЛиста.Автомобиль;
			ТекСтрока.Номер		 	= ДанныеПутевогоЛиста.Номер;
			ТекСтрока.Дата		 	= ДанныеПутевогоЛиста.Дата;
			ТекСтрока.ИД			= ДанныеПутевогоЛиста.ИД;	
		КонецЕсли;
