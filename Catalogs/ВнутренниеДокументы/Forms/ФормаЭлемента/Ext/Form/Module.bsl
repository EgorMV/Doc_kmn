...
Процедура РассчитатьОстатокДС()
	...
	ДвиженияДС.Вставить("СуммаНал", ?(КМН_СуммаНалАвансовыйОтчетДоходы>ЗапрошеноНал,0,ЗапрошеноНал-КМН_СуммаНалАвансовыйОтчетДоходы-КМН_СуммаОстатокНал));
	ДвиженияДС.Вставить("СуммаБезнал", ?(КМН_СуммаБезНалАвансовыйОтчетДоходы>ЗапрошеноБезнал,0,ЗапрошеноБезнал-КМН_СуммаБезНалАвансовыйОтчетДоходы-КМН_СуммаОстатокБезНал));
	ЗаполнитьСводФинансов(ДвиженияДС);
...
&НаКлиенте
Процедура РассчитатьСуточныеРасходы()
	
	ИскНоменклатура = ПолучитьНоменклатуру("Суточные");
	
	Если ИскНоменклатура<>Неопределено Тогда
		ИскСтроки = Объект.Товары.НайтиСтроки(Новый Структура("Номенклатура", ИскНоменклатура));
		Если ИскСтроки.Количество()>0 Тогда 
			ТекСтрока = ИскСтроки[0];
			ТекСтрока.Количество = КМН_СрокКомандировки;
	...
	//ИскРасход = КМН_ТаблицаАвансовыйОтчетРасходы.НайтиСтроки(Новый Структура("ВидРасхода", "Суточные"));
	//Если ИскРасход.Количество() > 0 Тогда
	//	ТекРасход = ИскРасход[0];
	//	ТекРасход.КоличествоЕстьЧек = КМН_СрокКомандировки;
	//	ОбработкаРеквизитаНаКлиенте("ЦенаЕстьЧек", ТекРасход);
	//КонецЕсли;
...
Процедура ПроверкаСоответствияСуточных()
	...
	Если ИскСтроки.Количество()>0 И ИскСтроки.Количество() Тогда
		...
		Элементы.НадписьРасхождениеВСуточных.Заголовок = СтрЗаменить(Элементы.НадписьРасхождениеВСуточных.Заголовок, "%1", РазницаСуточные);
